        ...
        <View style={{ flex: 1 }}>
          ...
          >
            <Text style={{ flexShrink: 1, fontWeight: 'bold', fontSize: 20, color: '#555' }}>
              {item && item.product_name && item.product_name.length > 100
    ...
    >
      ...
      >
        ...
        <View style={{ paddingHorizontal: 5, paddingVertical: 2, flexShrink: 1 }}>
          ...
          <Text style={{ fontSize: 12, fontStyle: 'italic', flexShrink: 1 }}>
        ...
        {item.buy_url != '' ? (
          ...
          >
            <TouchableOpacity
              onPress={() => {
                Amplitude.logEventWithPropertiesAsync('BUY URL FROM CONTEXT MODAL IN CATEGORY ', {
...
function User() {
  ...
  React.useEffect(() => {
    ...
    if (dataRetrieve && userId) {
      axios
        .get(
          `${URL}/feedsummary/byuser`,
          { params: { following_user_id: followingUserId, user_id: userId.slice(1, 13) } },
          { timeout: 5000 }
        )
        .then((res) => res.data)
        .then((responseData) => {
    ...
    axios
      .get(`${URL}/user/info`, { params: { user_id: followingUserId } }, { timeout: 5000 })
      .then((res) => res.data)
      .then((responseData) => {
        ...
        <TouchableOpacity style={{ marginLeft: 10, height: 30 }} disabled>
          ...
          followingUserInfo.user_profile_image != '' ? (
            ...
            <Avatar.Image
              style={{}}
              source={{
                uri: `https://ui-avatars.com/api/?rounded=true&name=${followingUserName}&size=64&background=D7354A&color=fff&bold=true`,
              }}
              size={30}
            />
          )}
        </TouchableOpacity>
        <TouchableOpacity style={{ marginLeft: 10, flex: 1 }} disabled>
