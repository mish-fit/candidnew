...
import { alttheme, background, borderColor, theme, themeLightest } from '../Exports/Colors';
        ...
        >
          ...
          >
            {item.user_profile_image ? (
              <Image
                source={{ uri: `${item.user_profile_image}?${moment().format('YYYY-MM-DD')}` }}
                style={[
              ...
              <Avatar.Image
                ...
                source={{
                  uri: `https://ui-avatars.com/api/?rounded=true&name=${item.following_user_name}&size=64&background=D7354A&color=fff&bold=true`,
                }}
                size={ITEM_SIZE - 40}
              />
            )}
          ...
          >
            ...
            >
              {item.following_user_name.length > 30
                ? `${item.following_user_name.substring(0, 20)}...`
                : item.following_user_name}
            </Text>
          </View>
          {item.count_posts > 0 ? (
            <View
            ...
            >
              <Text
                style={[
                  home.mainViewCarouselScrollableItemText,
                  { margin: 1, fontSize: 10, color: 'white' },
                ]}
              >
                {item.count_posts}
              </Text>
...
function FollowingCarousel({ DATA, isFollowing, onClickItem, onClickFollow }) {
  ...
  const renderItem = ({ item, index }) => {
    ...
    return (
      ...
      >
        <TouchableOpacity
          style={[home.mainViewCarouselScrollableItemButton, { borderWidth: 0, marginRight: 10 }]}
          onPress={() => {
            itemClick(item);
          }}
        >
          ...
          >
            <Text
              style={[
                home.mainViewCarouselScrollableItemText,
                { margin: 1, fontSize: 10, color: borderColor, textAlign: 'center' },
              ]}
            >
          ...
          <TouchableOpacity
            ...
            onPress={itemFollow}
            style={{
              backgroundColor: isFollowing[index] ? '#888' : themeLightest,
              borderRadius: 5,
              height: 20,
              justifyContent: 'center',
              alignItems: 'center',
          ...
          >
            <Text
              style={{
                fontSize: 10,
                color: isFollowing[index] ? 'white' : 'black',
                alignSelf: 'center',
              }}
            >
...
function TrendingProducts({ DATA, onClickItem }) {
  const [data, setData] = React.useState([...DATA]);
  const scrollX = React.useRef(new Animated.Value(0)).current;
        ...
        >
          ...
          >
            <Text
              style={[
                home.mainViewCarouselScrollableItemText,
                { margin: 1, fontSize: 14, color: borderColor },
              ]}
            >
...
function FeedAlt() {
    ...
    if (dataRetrieve) {
      ...
      firebase.auth().onAuthStateChanged((user) => {
        if (user) {
          ...
          axios
            .get(`${URL}/all/categories`, { params: { limit: 100 } }, { timeout: 5000 })
            .then((res) => res.data)
            .then((responseData) => {
              setLoading(false);
          ...
          axios
            .get(`${URL}/all/trending`, { timeout: 5000 })
            .then((res) => res.data)
            .then((responseData) => {
              setLoading(false);
              //  console.log("Categories",responseData)
          ...
          axios
            .get(`${URL}/feed/hero`, { timeout: 5000 })
            .then((res) => res.data)
            .then((responseData) => {
              setLoading(false);
              // console.log(responseData)
  ...
  const goToProduct = (id, name) => {
    ...
    navigation.navigate('ProductPage', { userInfo, product_name: name, product_id: id });
  };
  ...
  const onSearchHero = () => {
    axios
      .get(`${URL}/search/product`, { params: { product_text: heroSearchText } }, { timeout: 5000 })
      .then((res) => res.data)
  ...
  const followUser = (id, name, index) => {
    Amplitude.logEventWithPropertiesAsync('FOLLOW USER IN ALT FEED', {
      user_name: userInfo.user_name,
      ...
      >
        <ScrollView style={{ backgroundColor: 'white', borderRadius: 30, paddingBottom: 60 }}>
          ...
          <Text style={{ textAlign: 'center', fontSize: 20 }}>
            You earned 500 coins for signing up{' '}
          </Text>
          ...
          <View style={{ justifyContent: 'center', alignItems: 'center' }}>
            ...
            <TouchableOpacity
              style={[home.modalButton, { marginBottom: 20 }]}
              onPress={() => {
                setModalVisible(false);
                navigation.navigate('HowToEarn', { userInfo });
              }}
            >
        ...
        <View style={{ marginTop: headerHeight }}>
          ...
          >
            {/* <TextInput 
                ...
                /> */}
            <Text style={{ color: '#999' }}>Search for products here</Text>
          ...
          {peopleYouCanFollow.length ? (
            <View style={{ marginRight: 10, backgroundColor: '#FFF', marginBottom: 5 }}>
              <FollowingCarousel
                DATA={peopleYouCanFollow}
                isFollowing={isFollowing}
                onClickItem={(id, name, following) => goToUser(id, name, following)}
                onClickFollow={(id, name, index) => followUser(id, name, index)}
              />
            </View>
          ) : null}
          <Text
          ...
          {trendingProducts.length ? (
            <View style={{ marginRight: 10, backgroundColor: '#FFF', marginBottom: 5 }}>
              <TrendingProducts
                ...
                onClickItem={(id, name) => goToProduct(id, name)}
              />
            </View>
          ) : null}
        </View>
          ...
          {masterCategory.map((item, index) => (
            <Pressable
              ...
              style={{
                backgroundColor: 'white',
                borderRadius: 10,
                width: Dimensions.get('screen').width * 0.305,
                height: Dimensions.get('screen').width * 0.305,
                marginHorizontal: Dimensions.get('screen').width * 0.01,
