...
import { Animated, Easing, View,Text , Image ,ImageBackground, TouchableOpacity , TextInput , Dimensions , Button, ToastAndroid , ScrollView , PermissionsAndroid, Pressable, StyleSheet } from 'react-native'
...
const ProfileImage = () => {
    ...
    const [refereeId,setRefereeId] = React.useState(route.params?.refereeId && route.params?.refereeId != "" ? route.params?.refereeId : "")
    const [refereeName,setRefereeName] = React.useState(route.params?.refereeName && route.params?.refereeName != "" ? route.params?.refereeName : "")
    const [coinsValue,setCoinsValue] = React.useState(route.params?.coinsValue && route.params?.coinsValue != 0 ? route.params?.coinsValue : 0)
    ...
    const onSubmitOnboarding = () =>{
        ...
        const userbody = {
          ...
          "user_name": userName,
          "user_profile_image": profileImageChange ? s3URL + route.params?.phoneNumber.slice(1,13) + "/profile" : "",
    ...
    return (
        <View style = {{flex : 1 , backgroundColor : 'white'}}>
            ...
            flexDirection : 'row',  justifyContent : 'space-between', alignItems : 'center'}}>
                ...
                <TouchableOpacity
                    style = {{marginLeft : 0, marginRight:20, flex : 1 , justifyContent :'center', alignItems :'center' }}
                    disabled
                    >
            ...
            <View style = {{
                marginTop : 60 , justifyContent : 'center', alignItems : 'center', borderRadius : Dimensions.get('screen').width,
                width : Dimensions.get('screen').width, height : Dimensions.get('screen').width}}>
              <TouchableOpacity style = {{
                  position : 'absolute' , top : Dimensions.get('screen').width*0.1 , left : Dimensions.get('screen').width*0.1 , 
                  width : Dimensions.get('screen').width*0.8, height : Dimensions.get('screen').width*0.8,borderRadius : Dimensions.get('screen').width*0.8,
              }}
                
                onPress = {pickProfilePhoto}>
                <ImageBackground source = {image && image != "None"? {uri : image  } : {uri : 'https://ui-avatars.com/api/?rounded=true&name&size=512'}} 
                        style = {{width : Dimensions.get('screen').width*0.8, height : Dimensions.get('screen').width*0.8,
              ...
              {!image ? 
                <TouchableOpacity style = {{justifyContent : 'center' , alignItems : 'center'}} onPress={pickProfilePhoto}>
                    <Text style = {{color : "#555", fontWeight : 'bold', fontSize : 20 , }}>Pick a profile image</Text>
                </TouchableOpacity>
                : null
            }
            ...
            <View style = {{ marginTop : 30, width : Dimensions.get('screen').width*0.9, position : 'absolute' , bottom : 30 , right : 30 , 
                 alignItems:'flex-end'}}>
                <LinearGradient colors={submitted ?  ['rgba(200,200,200,0.5)','rgba(225,225,225,0.5)','rgba(250,250,250,0.5)'] : ["#ed4b60","#E7455A","#D7354A"]} style = {{
