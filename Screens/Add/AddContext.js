...
import { backArrow, background, theme } from '../Exports/Colors';
...
function AddContext() {
  ...
  const [productName, setProductName] = React.useState(
    route?.params?.product_name ? route?.params?.product_name : ''
  );
  const [productId, setProductId] = React.useState(
    route?.params?.product_id ? route?.params?.product_id : ''
  );
  ...
  const onClickSearchItemChild = (name) => {
    ...
    //  console.log(name)
    navigation.navigate('AddImage', { body, context_name: name });
  };
  ...
  const searchProduct = (text) => {
    ...
    axios
      .get(
        `${URL}/search/context`,
        { params: { context_text: text, category_name: body.category_name } },
        { timeout: 3000 }
      )
      .then((res) => res.data)
      .then((responseData) => {
    ...
    <View style={{ flex: 1, backgroundColor: background }}>
      <Animated.View style={add.headerView}>
        ...
        <TouchableOpacity style={add.headerTitle} disabled>
          <Text style={add.headerTitleText}>Add Context</Text>
        </TouchableOpacity>
      </Animated.View>
                ...
                >
                  {item.context_image && item.context_image !== 'None' ? (
                    ...
                    <Avatar.Image
                      ...
                      source={{
                        uri: `https://ui-avatars.com/api/?rounded=true&name=${item.context_name}&size=64&background=D7354A&color=fff&bold=true`,
                      }}
                      size={30}
                    />
                  )}
