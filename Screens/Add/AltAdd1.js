...
function AltAdd1() {
  ...
  const [productId, setProductId] = React.useState(
    route?.params?.product_id ? route?.params?.product_id : 0
  );
  ...
  const submit = () => {
    ...
    axios(
      ...
      .then((res) => {
        ...
        navigation.navigate('PostShare', { body });
      })
      .catch((e) => {
        ToastAndroid.show(
    ...
    <View style={{ flex: 1, backgroundColor: 'white' }}>
      ...
      >
        <View style={add.element}>
          <Text>
            <Text style={{ fontWeight: 'bold', marginTop: 20, color: alttheme, fontSize: 18 }}>
              Buy URL
            </Text>
            <Text style={{ color: alttheme, fontSize: 15 }}>
              {' '}
              ( You can earn coins when someone buy using this list from your post ){' '}
            </Text>
          </Text>
          <TextInput
        ...
        >
          <TouchableOpacity
            ...
            onPress={submit}
            style={{
              backgroundColor: submitted ? '#888' : theme,
              borderRadius: 5,
