...
function AddReview() {
  ...
  const onClickSearchItemChildCategory = (category_name, category_id) => {
    if (category_id > 0) {
      ...
      Amplitude.logEventWithPropertiesAsync('ADDED NEW CATEGORY', { category_name });
      setSearchTextCategory(category_name);
      setCategorySelected(true);
      setBody((body) => ({ ...body, category_name, category_id }));
      axios
        .get(
          `${URL}/search/context`,
          { params: { context_text: '', category_name } },
          { timeout: 3000 }
        )
        .then((res) => res.data)
        .then((responseData) => {
      ...
      setSearchTextCategory(category_name);
      setCategorySelected(true);
      setBody((body) => ({ ...body, category_name }));
    }
  };
  ...
  const searchCategory = (text) => {
    ...
    axios
      .get(`${URL}/search/category`, { params: { category_text: text } }, { timeout: 3000 })
      .then((res) => res.data)
      .then((responseData) => {
        setSearchCategoryLoading(false);
        setSearchCategoryArray(responseData);
    ...
    if (
      ...
      axios(
        ...
        .then((res) => {
          ...
          navigation.navigate('AltAdd1', { body, product_id: res.data });
        })
        .catch((e) => console.log(e));
    } else {
      navigation.navigate('AltAdd1', { body, product_id: body.product_id });
    }
  };
              ...
              >
                ...
                item.category_image != '' ? (
                  ...
                  <Avatar.Image
                    style={add.dropDownItemAvatar}
                    source={{
                      uri: `https://ui-avatars.com/api/?rounded=false&name=${item.category_name}&size=64&background=D7354A&color=fff&bold=true`,
                    }}
                    size={30}
                  />
                )}
