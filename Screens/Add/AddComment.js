...
function AddComment() {
  ...
  React.useEffect(() => {
    ...
    setFullBody({
      ...
      context_name: contextExists.context_name,
  ...
  const submit = () => {
    ...
    axios(
      ...
      .then((res) => {
        //   console.log(res)
        ToastAndroid.show(
          "Wohoo!! It's posted. Share it on your social pages and start earning coins.",
          ToastAndroid.SHORT
        );
    ...
    <View style={{ flex: 1, backgroundColor: background }}>
      <Animated.View style={add.headerView}>
        ...
        <TouchableOpacity style={add.headerTitle} disabled>
          <Text style={add.headerTitleText}>Critique</Text>
        </TouchableOpacity>
      </Animated.View>
      <View style={{ margin: 10, marginTop: 20 }}>
        ...
        <View style={add.buttonView}>
          <TouchableOpacity disabled={submitted} style={add.button} onPress={submit}>
